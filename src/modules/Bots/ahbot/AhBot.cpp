...
void AhBot::ForceUpdate()
{
    ...
    for (int i = 0; i < MAX_AUCTIONS; i++)
    {
        ...
        for (int j = 0; j < CategoryList::instance.size(); j++)
        {
            ...
            int categoryAnswered = Answer(i, category, &inAuctionItems);
            int categoryAdded = AddAuctions(i, category, &inAuctionItems);
            answered += categoryAnswered;
            added += categoryAdded;
            auctionAnswered += categoryAnswered;
            auctionAdded += categoryAdded;
        }

        sLog.outString("AhBot auction house %u finished. %d auctions answered, %d new auctions added",
                auctionIds[i], auctionAnswered, auctionAdded);
    }
...
int AhBot::AddAuctions(int auction, Category* category, ItemBag* inAuctionItems)
{
    ...
    sLog.outDetail("Category %s has %d auctions, maxAllowedAuctionCount=%d",
            category->GetDisplayName().c_str(), inAuctionItems->GetCount(category), maxAllowedAuctionCount);
    ...
    for (int32 i = 0; i <= maxAllowedAuctionCount && available.size() > 0 && inAuctionItems->GetCount(category) < maxAllowedAuctionCount; ++i)
    {
        ...
        else if (sellTime && time(0) - sellTime > sAhBotConfig.maxSellInterval)
        {
        ...
        sLog.outDetail("auction %d category '%s' - [%s]x%d to add (%d/%d count/allowed)",
                auction, category->GetDisplayName().c_str(), proto->Name1, auctionsToAdd,
...
int AhBot::AddAuction(int auction, Category* category, ItemPrototype const* proto)
{
    ...
    {
        sLog.outDetail("AhBot cannot add %s to auction %d: owner %u has no character name",
                proto->Name1, auctionIds[auction], owner);
        return 0;
    }
    ...
    if (!price || !stackCount)
    {
        sLog.outDetail("AhBot cannot add %s to auction %d: price=%u stackCount=%u",
                proto->Name1, auctionIds[auction], price, stackCount);
        return 0;
    }
    ...
    if (!item)
    {
        sLog.outDetail("AhBot cannot add %s to auction %d: item creation failed",
                proto->Name1, auctionIds[auction]);
        return 0;
    }
    ...
    if(!ahEntry)
    {
        sLog.outDetail("AhBot cannot add %s to auction %d: auction house entry missing",
                proto->Name1, auctionIds[auction]);
        return 0;
    }
...
void AhBot::LoadRandomBots()
{
    for (list<uint32>::iterator i = sPlayerbotAIConfig.randomBotAccounts.begin(); i != sPlayerbotAIConfig.randomBotAccounts.end(); i++)
    {
        ...
        do
        {
            ...
            bool horde = race == RACE_ORC || race == RACE_UNDEAD ||
                    race == RACE_TAUREN || race == RACE_TROLL;
            bidders[horde ? 2 : 1].push_back(guid);
    ...
    sLog.outString("{A=%d,H=%d,N=%d} AhBot bidders loaded", bidders[1].size(), bidders[2].size(), bidders[3].size());
}
