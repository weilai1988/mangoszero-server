...
#include "Ai/Base/BotBrainFactory.h"
#include "DBCStores.h"
...
namespace
{
    const char* BotBrainRaidMarkerName(uint8 index)
    {
        switch (index)
        {
            case 0: return "star";
    ...
    Unit* GetBotBrainMarkerUnit(Player* bot, Group* group, uint8 index)
    {
        if (!bot || !group || !bot->GetMap())
            return NULL;

        ObjectGuid markerGuid = group->GetTargetIcon(index);
        if (!markerGuid)
            return NULL;

        return bot->GetMap()->GetUnit(markerGuid);
    }
}
...
PlayerbotAI::PlayerbotAI() : PlayerbotAIBase(), bot(NULL), aiObjectContext(NULL),
    brain(NULL), currentEngine(NULL), chatHelper(this), chatFilter(this), accountId(0), security(NULL), master(NULL), currentState(BOT_STATE_NON_COMBAT)
{
    ...
    PlayerbotAIBase(), chatHelper(this), chatFilter(this), security(bot), master(NULL)
{
    ...
    brain = BotBrainFactory::Create(bot, this);
...
{
    if (EmotesTextEntry const* emoteEntry = sEmotesTextStore.LookupEntry(emote))
    {
        uint32 emoteId = emoteEntry->textid;
        time_t lastSaid = sounds[emoteId];
        if (!lastSaid || (time(0) - lastSaid) >= sPlayerbotAIConfig.soundRepeatDelay / 1000)
        {
            sounds[emoteId] = time(0);
...
BotBrainContext PlayerbotAI::BuildBrainContext()
{
    BotBrainContext brainContext;
    brainContext.hasMaster = master != NULL;
    brainContext.inCombat = bot->IsInCombat() || currentState == BOT_STATE_COMBAT;

    if (Group* group = bot->GetGroup())
    {
        brainContext.isInGroup = true;
        brainContext.groupSize = group->GetMembersCount();
        brainContext.inRaid = group->isRaidGroup();
        brainContext.partyPhase = BOT_BRAIN_PARTY_PREPARE;

        bool groupInCombat = false;
        float weakestHealth = 101.0f;
        ...
        for (Group::member_citerator i = slots.begin(); i != slots.end(); ++i)
        {
            ...
            if (member->IsAlive())
            {
                ++brainContext.aliveMemberCount;
                float memberHealth = member->GetHealthPercent();
                if (memberHealth < weakestHealth)
                {
                    weakestHealth = memberHealth;
                    brainContext.weakestMemberName = member->GetName();
                    brainContext.weakestMemberHealthPercent = memberHealth;
                }
            }
            else
            {
                ++brainContext.deadMemberCount;
            }
            ...
            if (IsTank(member))
            {
                ++brainContext.tankCount;
                if (brainContext.tankName.empty())
                    brainContext.tankName = member->GetName();
            }
            else if (IsHeal(member))
            {
                ++brainContext.healerCount;
            }
            else
            {
                ++brainContext.dpsCount;
            }
        }

        uint8 mainMarkers[] = { 7, 6, 5 };
        for (uint32 i = 0; i < sizeof(mainMarkers) / sizeof(mainMarkers[0]); ++i)
        {
            ...
            if (markerTarget && markerTarget->IsAlive())
            {
                brainContext.mainTargetMarker = BotBrainRaidMarkerName(mainMarkers[i]);
                brainContext.mainTargetName = markerTarget->GetName();
                break;
            }
        }

        uint8 ccMarkers[] = { 4, 3, 2, 5 };
        for (uint32 i = 0; i < sizeof(ccMarkers) / sizeof(ccMarkers[0]); ++i)
        {
            Unit* markerTarget = GetBotBrainMarkerUnit(bot, group, ccMarkers[i]);
            if (markerTarget && markerTarget->IsAlive())
            {
                brainContext.crowdControlMarker = BotBrainRaidMarkerName(ccMarkers[i]);
                brainContext.crowdControlTargetName = markerTarget->GetName();
                break;
            }
        }

        brainContext.inCombat = brainContext.inCombat || groupInCombat;

        if (brainContext.deadMemberCount)
            brainContext.partyPhase = BOT_BRAIN_PARTY_RECOVER;
        else if (groupInCombat || brainContext.inCombat)
            brainContext.partyPhase = BOT_BRAIN_PARTY_FIGHT;
        else if (!brainContext.mainTargetName.empty())
            brainContext.partyPhase = BOT_BRAIN_PARTY_PULL;
    }

    if (Map* map = bot->GetMap())
    {
        brainContext.inDungeon = map->IsDungeon();
        brainContext.inRaid = brainContext.inRaid || map->IsRaid();
        if (const char* mapName = map->GetMapName())
            brainContext.zoneName = mapName;
    }
    ...
    {
        if (zone->area_name[0])
            brainContext.zoneName = zone->area_name[0];
    }

    if (bot->GetMaxHealth())
        brainContext.healthPercent = bot->GetHealthPercent();

    if (bot->GetPowerType() == POWER_MANA && bot->GetMaxPower(POWER_MANA))
        brainContext.manaPercent = (bot->GetPower(POWER_MANA) * 100.0f) / bot->GetMaxPower(POWER_MANA);

    if (IsTank(bot))
        brainContext.role = BOT_BRAIN_ROLE_TANK;
    else if (IsHeal(bot))
        brainContext.role = BOT_BRAIN_ROLE_HEALER;
    else
        brainContext.role = BOT_BRAIN_ROLE_DPS;

    Unit* currentTarget = aiObjectContext ? aiObjectContext->GetValue<Unit*>("current target")->Get() : NULL;
    if (currentTarget)
    {
        brainContext.currentTargetName = currentTarget->GetName();
        brainContext.threatPercent = aiObjectContext->GetValue<uint8>("threat", "current target")->Get();
    }

    Unit* tankTarget = aiObjectContext ? aiObjectContext->GetValue<Unit*>("tank target")->Get() : NULL;
    if (tankTarget)
        brainContext.tankTargetName = tankTarget->GetName();

    Unit* partyHealTarget = aiObjectContext ? aiObjectContext->GetValue<Unit*>("party member to heal")->Get() : NULL;
    if (partyHealTarget)
    {
        brainContext.partyHealTargetName = partyHealTarget->GetName();
        brainContext.partyHealTargetHealthPercent = partyHealTarget->GetHealthPercent();
    }

    if (!brainContext.currentTargetName.empty())
        brainContext.encounterName = brainContext.currentTargetName;

    return brainContext;
}

BotDecision PlayerbotAI::SelectBrainDecision()
{
    if (!brain)
        return BotDecision();

    return brain->SelectNextDecision(BuildBrainContext());
}

string PlayerbotAI::FormatBrainState()
{
    BotBrainContext brainContext = BuildBrainContext();
    BotDecision decision = brain ? brain->SelectNextDecision(brainContext) : BotDecision();

    ostringstream out;
    out << "Brain=" << (brain ? brain->GetName() : "none")
        << " role=" << BotBrainRoleName(brainContext.role)
        << " phase=" << BotBrainPartyPhaseName(brainContext.partyPhase)
        << " group=" << (brainContext.isInGroup ? brainContext.groupSize : 0)
        << (brainContext.inRaid ? "/raid" : (brainContext.inDungeon ? "/dungeon" : "/world"))
        << " roles=T" << brainContext.tankCount
        << "/H" << brainContext.healerCount
        << "/D" << brainContext.dpsCount
        << " alive=" << brainContext.aliveMemberCount
        << " dead=" << brainContext.deadMemberCount
    ...
    out << " threat=" << uint32(brainContext.threatPercent)
        << "% target=" << (brainContext.currentTargetName.empty() ? "none" : brainContext.currentTargetName)
        << " main=" << (brainContext.mainTargetMarker.empty() ? "none" : brainContext.mainTargetMarker)
        << ":" << (brainContext.mainTargetName.empty() ? "none" : brainContext.mainTargetName)
        << " cc=" << (brainContext.crowdControlMarker.empty() ? "none" : brainContext.crowdControlMarker)
        << ":" << (brainContext.crowdControlTargetName.empty() ? "none" : brainContext.crowdControlTargetName)
        << " tank=" << (brainContext.tankName.empty() ? "none" : brainContext.tankName)
        << " tankTarget=" << (brainContext.tankTargetName.empty() ? "none" : brainContext.tankTargetName)
        << " weak=" << (brainContext.weakestMemberName.empty() ? "none" : brainContext.weakestMemberName);

    if (!brainContext.weakestMemberName.empty())
        out << "(" << uint32(brainContext.weakestMemberHealthPercent) << "%)";

    out
        << " healTarget=" << (brainContext.partyHealTargetName.empty() ? "none" : brainContext.partyHealTargetName);

    if (!brainContext.partyHealTargetName.empty())
        out << "(" << uint32(brainContext.partyHealTargetHealthPercent) << "%)";

    out
        << " intent=" << BotBrainIntentName(decision.intent)
        << ":" << (decision.action.empty() ? "none" : decision.action);
...
void PlayerbotAI::InterruptSpell()
{
    for (int type = CURRENT_MELEE_SPELL; type < CURRENT_CHANNELED_SPELL; type++)
    {
        ...
        data << uint8(SPELL_FAILED_INTERRUPTED);
