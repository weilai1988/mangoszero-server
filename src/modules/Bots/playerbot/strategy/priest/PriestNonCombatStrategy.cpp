...
void PriestNonCombatStrategy::InitTriggers(std::list<TriggerNode*> &triggers)
{
	...
	triggers.push_back(new TriggerNode(
		"party member dead",
		NextAction::array(0, new NextAction("resurrection", ACTION_EMERGENCY + 4), NULL)));
}
...
void PriestBuffStrategy::InitTriggers(std::list<TriggerNode*> &triggers)
{
    ...
    triggers.push_back(new TriggerNode(
        ...
        NextAction::array(0, new NextAction("power word: fortitude on party", 36.0f), NULL)));

    triggers.push_back(new TriggerNode(
        "divine spirit on party",
        NextAction::array(0, new NextAction("divine spirit on party", 37.0f), NULL)));
}
