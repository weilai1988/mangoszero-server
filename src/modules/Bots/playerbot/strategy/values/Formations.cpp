...
namespace ai
{
    ...
    class StackFormation : public MoveAheadFormation
    {
    public:
        StackFormation(PlayerbotAI* ai) : MoveAheadFormation(ai, "stack") {}
        virtual WorldLocation GetLocationInternal()
        {
            Player* master = GetMaster();
            if (!master)
        ...
        virtual float GetMaxDistance() { return 0.50f; }
    };
    ...
    public:
        ...
        virtual WorldLocation GetLocationInternal()
        {
            ...
            WorldLocation loc = MoveLine(players, 0.0f, x, y, z, orientation, range);
            if (!Formation::IsNullLocation(loc))
                return loc;

            float angle = GetFollowAngle();
            float lx = master->GetPositionX() + cos(angle) * range;
            float ly = master->GetPositionY() + sin(angle) * range;
            float lz = master->GetPositionZ();
            float ground = master->GetMap()->GetHeight(
#ifdef MANGOSBOT_TWO
                bot->GetPhaseMask(),
#endif
            ...
            return WorldLocation(master->GetMapId(), lx, ly, lz);
        }
    };
...
bool SetFormationAction::Execute(Event event)
{
    ...
    if (!value->Load(formation))
    {
        ...
        ai->TellMaster(str);
        ai->TellMaster("Please set to any of:|cffffffff stack, near (default), queue, chaos, circle, line, shield, arrow, melee, far");
        return false;
    }

    if (formation == "stack" || formation == "near" || formation == "default" || formation == "line" ||
            formation == "shield" || formation == "arrow" || formation == "melee" ||
            formation == "queue")
    {
        ai->ChangeStrategy("+follow,-stay,-runaway,-passive", BOT_STATE_NON_COMBAT);
        ai->ChangeStrategy("+follow,-stay,-runaway,-ranged,-flee", BOT_STATE_COMBAT);
    }
