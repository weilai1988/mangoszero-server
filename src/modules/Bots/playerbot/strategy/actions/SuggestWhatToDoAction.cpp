        ...
        if (urand(0, 10) < 3) {
            ...
            for (int j = 0; j < ahbot::CategoryList::instance.size(); j++)
            {
                ...
                if (name == category->GetName())
                {
                    ...
                    transform(item.begin(), item.end(), item.begin(), [](unsigned char c) { return char(tolower(c)); });
                    ostringstream itemout;
...
void SuggestWhatToDoAction::spam(string msg, uint32 channelId)
{
    ...
    Player::JoinedChannelsList const& channels = to->GetJoinedChannels();
    for (Player::JoinedChannelsList::const_iterator i = channels.begin(); i != channels.end(); ++i)
    {
        Channel* chn = *i;
