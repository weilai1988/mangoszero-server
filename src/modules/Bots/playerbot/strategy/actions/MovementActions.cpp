...
bool MovementAction::Flee(Unit *target, float distance)
{
    Player* master = GetMaster();
    if (!target)
        target = master;
    ...
    FleeManager manager(bot, distance, bot->GetAngle(target) + M_PI);
    if (!manager.isUseful())
        return false;
...
{
    return Flee(AI_VALUE(Unit*, "current target"), distance);
}
